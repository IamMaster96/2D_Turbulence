...
read(10,*)
read(10,*)particles,nparticle,nstokes,base_d,nrunpart,part_t
close(10)
...
if(particles)then
        ...
        allocate(send_ly(1:nparticle),send_lx(1:nparticle))
        allocate(send_uy(1:nparticle),send_ux(1:nparticle))
        allocate(send_lvy(1:nparticle),send_lvx(1:nparticle),send_utrac(1:nparticle))
        allocate(send_uvy(1:nparticle),send_uvx(1:nparticle),send_ltrac(1:nparticle))
        ...
        allocate(taup(1:nstokes),partsize(nstokes),spartsize(nstokes),meandi(1:nstokes))
        open(unit=11,file='taup.in',status='old')
        do istokes=1,nstokes
                read(11,*)taup(istokes)
        end do
        do istokes=1,nstokes
                meandi(istokes)=base_d*taup(istokes)**0.5d0
        end do
        close(11)
end if
!!!!!!!!!!!!!particle code!!!!!!!!!!!!
...
do iiter=1,maxiter/navg
        do iiner=1,navg
                ...
                !do i=1,sizex
                !print *, xpart(1,i),xpart_trac(1,i),rank
                !end do
                if(rank==0)then
